...
jQuery(function($) {
	...
	function enableDemoAjax() {
		...
		var demo_ajax_options = {
			...
			'default_url' : '#/demand/manager/findList.html',// default hash
			'content_url' : function(hash) {
				...
				// and load it
				if (path.match(/(\/resources\/)(index\.shtml)?/)) {
					if (hash.indexOf("\/") == 0) {
						return hash;
					} else {
						return path.replace(/(index\.shtml)/, hash.replace(/^page\//, '')+ '.html');
					}
				}
	...
	function smallDeviceDropdowns() {
		...
		$(document).on('shown.bs.dropdown.navbar',
				'.ace-nav > li.dropdown-modal', function(e) {
	...
	$$.getAttrSettings = function(elem, attr_list, prefix) {
		...
		for ( var li in attr_list)
			if (attr_list.hasOwnProperty(li)) {
				...
				if (!((attr_val = elem.getAttribute(prefix + attr_name))))
					continue;
