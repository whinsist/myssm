...
jQuery(function($) {
	...
	*/ 
	jQuery(grid_selector).jqGrid({
		caption: "商品列表",
		editurl: "/admin/decItem/deleteOrUpdate.do",
		url: "/admin/decItem/listJson.do",
		mtype: "POST",
		datatype: "json",
		...
		colModel:[
			...
			{label:'商品名',name:'name',index:'name',editable:true,sortable:false,formatter:userNameFormat},
			{label:'价格',name:'price',index:'price',editable:false},
			{label:'库存',name:'num',index:'num',editable:true,sortable:false},
			...
			{label:'新增时间',name:'created_str',index:'created_str',editable:false,sortable:false},
			{label:'更新时间',name:'updated_str',index:'updated_str',editable:false,sortable:false},
			{label:'ID',name:'id',index:'id', hidden:true, key:true, editable: false,sortable:false}
		],
		viewrecords:true,
		rowNum:15,
	...
	jQuery(grid_selector).jqGrid('navGrid',pager_selector,
		{ 
			...
			addfunc : function(e){
				window.location.href = ctx+"/admin/decItem/add.do";
			},
			del: true,
			...
			delfunc : function(e){
				...
				$("#dialog-confirm-content").html("<i class='ace-icon fa fa-hand-o-right blue bigger-120'></i>你确定要删除选择的"+selectIds.length+"个商品?");
					...
					buttons: [
						{
							...
							click: function() {
								$.ajax({
							        type: "POST",
							        url: '/admin/decItem/deleteOrUpdate.do',
	...
	function statusFormat(cellvalue, options, rowObject){
		var statusMap = {"Publish":"发布", "Delete":"删除"}; 
