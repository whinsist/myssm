...
public class Pems {
        ...
        return fromPem(supplier,
                new PemProcessor<KeySpec>(ImmutableMap.of(
                        ...
                        new PemProcessor.ResultParser<KeySpec>() {
                            ...
                            @Override
                            public X509EncodedKeySpec parseResult(byte[] bytes) {
     ...
     * @throws CertificateException the certificate exception
     */
    public static X509Certificate x509Certificate(ByteSource supplier, CertificateFactory certFactory)
            throws IOException, CertificateException {
        final CertificateFactory certs = certFactory != null ? certFactory : CertificateFactory.getInstance("X.509");
        try {
            return fromPem(supplier,
                    new PemProcessor<X509Certificate>(ImmutableMap.of(
                            CERTIFICATE_X509_MARKER,
                            new PemProcessor.ResultParser<X509Certificate>() {

                                @Override
                                public X509Certificate parseResult(byte[] bytes) {
                                    try {
    ...
    public static String pem(PrivateKey key) {
        ...
        return pem(key instanceof RSAPrivateCrtKey ? ASN1Codec.encode(RSAPrivateCrtKey.class.cast(key)) : key
                .getEncoded(), marker);
    }
