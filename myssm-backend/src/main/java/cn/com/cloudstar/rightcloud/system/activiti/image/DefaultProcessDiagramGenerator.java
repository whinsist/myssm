...
public class DefaultProcessDiagramGenerator implements ProcessDiagramGenerator {
    ...
    private static ThreadLocal<List<FlowRecord>> FLOWRECORDS = new ThreadLocal<>();
    ...
    /**
     * The instructions on how to draw a certain construct is created statically and stored in a map
     * for performance.
     */
    ...
    protected DefaultProcessDiagramCanvas generateProcessDiagram(BpmnModel bpmnModel, String imageType,
        ...
        processDiagramCanvas.setHighLightedActivities(highLightedActivities);
        processDiagramCanvas.setHighLightedFlows(highLightedFlows);
        ...
        List<FlowRecord> flowRecords = FLOWRECORDS.get();
        if (flowRecords != null && flowRecords.size() > 0) {
            // 重新绘制高亮显示的连接线
            for (FlowRecord record : flowRecords) {
                processDiagramCanvas.drawSequenceflow(record.xPoints, record.yPoints,
                        record.drawConditionalIndicator, record.isDefault,
                        record.highLighted, record.scaleFactor);
            }
            FLOWRECORDS.remove();
        }
    ...
    protected void drawActivity(DefaultProcessDiagramCanvas processDiagramCanvas, BpmnModel bpmnModel,
        ...
        if (drawInstruction != null) {
            ...
            /*if (highLightedActivities.contains(flowNode.getId())) {
                int index = highLightedActivities.indexOf(flowNode.getId());
                Boolean finished = ProcessConstants.PROCESS_FINISHED.get();
                if (index == highLightedActivities.size() - 1 && !finished) {
                    drawHighLightCurrent(processDiagramCanvas, bpmnModel.getGraphicInfo(flowNode.getId()));
                } else {
        ...
        for (SequenceFlow sequenceFlow : flowNode.getOutgoingFlows()) {
            ...
            if (graphicInfoList != null && graphicInfoList.size() > 0) {
                ...
                if (highLighted) {
                    // 高亮显示的连接线可能被后续绘制覆盖，这里记录下高亮显示的连接线，在最后进行重新绘制
                    List<FlowRecord> flowRecords = FLOWRECORDS.get();
                    if (flowRecords == null) {
                        flowRecords = Lists.newArrayList();

                        FLOWRECORDS.set(flowRecords);
                    }

                    flowRecords.add(new FlowRecord(xPoints, yPoints, drawConditionalIndicator, isDefault, highLighted, scaleFactor));
                }
                ...
                if (labelGraphicInfo != null) {
                    ...
                    GraphicInfo textG = new GraphicInfo();

                    GraphicInfo point1 = graphicInfoList.get(0);
                    GraphicInfo point2 = graphicInfoList.get(1);
                    if (point2.getY() - point1.getY() > 10) {
                        textG.setX(point1.getX() + 2);
                        textG.setY(point1.getY() + 40);
                    } else {
                        textG.setX(point1.getX() + 5);
                        textG.setY(point1.getY());
                    }

                    processDiagramCanvas.drawLabel(sequenceFlow.getName(), textG, false);

                    //GraphicInfo lineCenter = getLineCenter(graphicInfoList);
                    /*int pos = graphicInfoList.size();
                    if (pos == 2) {
                        processDiagramCanvas.drawLabelOffset20(sequenceFlow.getName(), lineCenter, false);
                    } else {
    ...
    /**
     * This method returns shape type of base element.<br> Each element can be presented as
     * rectangle, rhombus, or ellipse.
     *
    ...
    private static void drawHighLightCurrent(DefaultProcessDiagramCanvas processDiagramCanvas, GraphicInfo graphicInfo) {
        processDiagramCanvas.drawHighLightCurrent((int) graphicInfo.getX(), (int) graphicInfo.getY(), (int) graphicInfo.getWidth(), (int) graphicInfo.getHeight());
