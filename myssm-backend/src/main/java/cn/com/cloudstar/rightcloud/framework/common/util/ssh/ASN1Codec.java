...
final class ASN1Codec {
    ...
    private static Optional<Object> buildObject(int tag, int tagNo, InputStream in, int limit) {
        ...
        if (isConstructed && tagNo == SEQUENCE) {
            return Optional.of(buildEncodableList(limited, limit));
        }
        byte[] bytes = toArray(limited);
        switch (tagNo) {
            case BIT_STRING:
                return Optional.of(nestedASN1Sequence(bytes));
            case INTEGER:
    ...
    static byte[] encode(RSAPrivateCrtKey key) {
        List<BigInteger> seq = ImmutableList.<BigInteger>builder().add(BigInteger.valueOf(0)) // version
                .add(key.getModulus()).add(key.getPublicExponent())
                .add(key.getPrivateExponent()).add(key.getPrimeP())
                .add(key.getPrimeQ()).add(key.getPrimeExponentP())
        ...
        Builder<Byte> output = ImmutableList.builder();
